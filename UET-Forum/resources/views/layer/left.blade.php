<div class="col-md leftBar">
    <button class="btn btn-primary btn-block" data-toggle="modal" data-target="#taophong" data-whatever="@mdo"><i
                class="fas fa-plus-circle"></i> Tạo mới
    </button>
    <!--Khung tao phong-->
                    ...
                    <nav>
                        <div class="nav nav-tabs" id="nav-tab" role="tablist">
                            <a class="nav-item nav-link active" id="nav-question-tab" data-toggle="tab"
                               href="#nav-question" role="tab" aria-controls="nav-question" aria-selected="true">Phiên
                                hỏi đáp</a>
                            <a class="nav-item nav-link" id="nav-form-tab" data-toggle="tab" href="#nav-form" role="tab"
                               aria-controls="nav-form" aria-selected="false">Phiếu khảo sát</a>
                        </div>
                    </nav>
                        ...
                        <form id="survey-post" action="{{route('survey-post')}}" method="post">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <label class="col-form-label">Mô tả:</label>
                                    <textarea form="survey-post" name="describeSur" class="form-control"
                                              rows="3"></textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
    ...
    <nav class="nav flex-column bigMenu">
        ...
        <a class="nav-link"
           href="{{route('dashboard-search',['admin'=>decrypt($_COOKIE['id']),'category_id'=>(!empty($conditions['category_id']))?$conditions['category_id']:null])}}"><i
                    class="fas fa-question-circle"></i> Phiên của tôi</a>
        <a class="nav-link" href="{{route('my-survey')}}"><i class="fas fa-check-square"></i> Phiếu khảo sát của tôi</a>
        @if($cUser['role_id']==1)   <a class="nav-link" href="{{route('user-list')}}"><i class="fa fa-address-card"></i>Danh
            sách User</a> @endif
    </nav>
</div>